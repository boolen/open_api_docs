...
用户可以通过`【短信页面】-【短信设置】-【发送授权】`来创建 SMS_USER, 同时 ConverLab 会自动生成对应的 SMS_KEY. 目前 ConverLab 只允许拥有一个 SMS_USER.
...
1. 在短信模板中使用变量, 作为占位符
2. 在短信 API 中设置变量的值
3. ConverLab 会根据请求参数, 来替换短信内容中相应变量的值
...
* 用户在 ConverLab 的 `【短信】- 【短信设置】-【SMSHook】` 中选择关注的事件, 配置接收数据的 URL
